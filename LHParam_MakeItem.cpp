...
bool LHParam_MakeItem::LoadData()
{
	...
	{
		assert( false && "MakeItem 파일을 찾을수 없습니다." );
		return false;
	}
...
bool LHParam_MakeItem::CheckNeedItemBuff( __int64 & rnNeedLaim ) // System Unlock Money
{
	assert( NULL != g_pRoh );
 ...
 // Display Lime on NPC
bool LHParam_MakeItem::CheckNeedItem( __int64 & rnNeedLaim ) // System Unlock Money
{
	assert( NULL != g_pRoh );
